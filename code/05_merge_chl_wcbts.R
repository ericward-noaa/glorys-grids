...
dataset <- rep("cmems_obs-oc_glo_bgc-plankton_my_l4-multi-4km_P1M")
...
for(ii in 1:length(year_range)) {
  ...
  haul_year <- haul_year %>%
    ...
    mutate(
      ...
      value = weighted.mean(temp$CHL[c(loc_d1,loc_d2,loc_d3)], w = c(w1, w2, w3))
    )
...
saveRDS(all_dat, "indices/WCBTS_chl_by_haul.rds")
