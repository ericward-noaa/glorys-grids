...
haul <- readRDS("norway.rds") # replace with whatever haul dataset to include
haul$month <- month(as.Date(haul$date))
haul$yday <- yday(as.Date(haul$date))
haul <- sdmTMB::add_utm_columns(haul, ll_names = c("lon","lat"))
...
depth_bins <- data.frame("depth_bin" = 1:9, 
                         "depth_min" = c(55, 109, 155, 222, 266, 318, 380, 454, 541) - 5,
                         "depth_max" = c(55, 109, 155, 222, 266, 318, 380, 454, 541) + 5)

depth_years <- expand.grid(depth_bin = 1:9, years = year_range)
...
for(ii in 1:nrow(depth_years)) {
  ...
  haul_year$day <- day(as.Date(haul_year$date))
  # Find grid cell closest to this location
  haul_year <- haul_year %>%
    rowwise() %>%
    mutate(
      this_loc = which.min((X - temp$X)^2 + (Y - temp$Y)^2),
      month_d1 = abs(difftime(date, ymd(paste0(yrs,"-",month-1,"-15")), units = "days")),# dist to month prior
      month_d2 = abs(difftime(date, ymd(paste0(yrs,"-",month,"-15")), units = "days")),# dist to mid of current
      month_d3 = abs(difftime(date, ymd(paste0(yrs,"-",month+1,"-15")), units = "days")),# dist to mid of next
  ...
  if(ii==1) {
    all_dat <- haul_year[,c("serialnumber","year","date","lat","lon", "thetao", "uo", "vo", "depth")]
  } else {
    all_dat <- rbind(all_dat, haul_year[,c("serialnumber","year","date","lat","lon","thetao", "uo", "vo", "depth")])
  }
  # clean up
  file.remove("CMEMS_tmp.nc")
}

saveRDS(all_dat, "indices/norwegian_temp_by_haul.rds")

# interpolate depth to each observation
haul <- dplyr::rename(haul, depth_m = depth) %>%
  dplyr::select(serialnumber, depth_m) %>%
  dplyr::group_by(serialnumber) %>%
  dplyr::summarise(depth_m = depth_m[1])
all_dat <- dplyr::left_join(all_dat, haul)
